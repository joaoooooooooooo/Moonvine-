...
type PieItemStyle = {
  color: string | echarts.graphic.LinearGradient | ImagePatternObject;
  opacity: number;
  borderRadius: number;
  borderColor?: string;
  borderWidth?: number;
...
export type PieVariant = "gradient" | "hatched";
export type EntityRole = "subject" | "comparison";
...
export interface PieProps {
  ...
  roleKey?: string; // optional row key holding a semantic entity role for each sector
...
function resolveSectorVariant(
  pie: PieSlot,
  row: Record<string, unknown>,
): PieVariant {
  if (!pie.roleKey) return pie.variant;
  const role = row[pie.roleKey];
  if (role === "comparison") return "hatched";
  if (role === "subject") return "gradient";
  return pie.variant;
}

function sectorVariantPaint(
  variant: PieVariant,
  slots: string[],
): string | echarts.graphic.LinearGradient | ImagePatternObject {
  if (variant === "hatched") {
    return patternFill("hatched", slots[0] ?? FALLBACK_COLOR) ?? withAlpha(slots[0] ?? FALLBACK_COLOR, 0.24);
  }

  return sectorPaint(slots);
}
...
function buildPieSeries(ctx: OptionBuildContext): PieSeriesOption[] {
  ...
  const sectors = data.map((row) => {
    ...
    const variant = resolveSectorVariant(pie, row);
    ...
    const itemStyle: PieItemStyle = {
      color: sectorVariantPaint(variant, slots),
